...
if (isset($_POST['delete_announcement'])) {
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Announcement deleted successfully.'); window.location.href = window.location.href;</script>";
    } else {
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // 🔽 Delete button form
        echo "<form method='POST' action='' onsubmit='return confirm(\"Are you sure you want to delete this announcement?\");' style='display:inline;'>";
        echo "<input type='hidden' name='delete_id' value='" . $row['id'] . "'>";
        echo "<button type='submit' name='delete_announcement' class='btn btn-sm btn-danger ml-2'>Delete</button>";
        echo "</form>";
