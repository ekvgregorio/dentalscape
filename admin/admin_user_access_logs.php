        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div>
                        <h1 class="card-title">Users Activity Records</h1>
                        <h5 class="text-primary mt-2"><h5 class="text-primary mt-2">Weekly User Login Count: <?php echo $weeklyCount; ?></h5></h5>
                    </div>
                    <div class="dropdown">
                        <button class="btn btn-sm btn-light bg-white dropdown-toggle" type="button" id="userDropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            <i class="mdi mdi-filter"></i> Login Status
                        </button>
              ...
              <div class="table-responsive" style="max-height: 500px; overflow-y: auto;">
                <table id="appointmentTable" class="display expandable-table" style="width:100%">
                    ...
                    <tbody>
                        <?php 
                            $loginHistory = getLoginHistory($conn, $status, $offset, $perPage);
                            if ($loginHistory) {
                                while ($record = $loginHistory->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <span class="status-badge <?php echo ($record['status'] == 'success') ? 'bg-success' : 'bg-danger'; ?> text-white">
                                                <?php echo ucfirst($record['status']); ?>
                                            </span>
                                        </td>
                                        <td><?php echo htmlspecialchars($record['latitude']); ?></td>
                                        <td><?php echo htmlspecialchars($record['longitude']); ?></td>
                                        <td><?php echo htmlspecialchars($record['neighbourhood']); ?>, <?php echo htmlspecialchars($record['municipality']); ?></td>
                                        <td><?php echo date('M d, Y', strtotime($record['login_datetime'])); ?></td>
                                        <td><?php echo date('H:i', strtotime($record['login_datetime'])); ?></td>
                                    </tr>
                                <?php endwhile;
                            } else {
