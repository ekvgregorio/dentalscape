...
function countTotalConfirmed($conn, $status = 'Confirmed') {
...
$totalConfirmedAppointments = countTotalConfirmed($conn);
            ...
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div>
                        <h1 class="card-title">Confirmed Appointments</h1>
                        <h5 class="text-primary mt-2">
                            Total Confirmed Appointments: <?php echo htmlentities($totalConfirmedAppointments, ENT_QUOTES, 'UTF-8'); ?>
                        </h5>
              ...
              <div class="table-responsive" style="max-height: 500px; overflow-y: auto;">
                <table id="appointmentTable" class="display expandable-table" style="width:100%">
                        ...
                        <tbody>
                            <?php
                                $appointments = getConfirmedAppointments($conn, $month_range, 'Confirmed', $offset, $perPage);
                                if ($appointments) {
                                  while ($record = $appointments->fetch_assoc()): ?>
                                    <tr style="cursor: pointer;" onmouseover="this.style.backgroundColor='#e6e6e6';" onmouseout="this.style.backgroundColor='';">
                                      ...
                                      <td><?php echo date('m-d-y H:i', strtotime($record['last_modified'])); ?></td>
                                      <td>
                                        <form method="POST" action="/dentalscape/admin/admin_process.php">
                                          <input type="hidden" name="appointment_id" value="<?php echo $record['id']; ?>">
                                              <button type="submit" name="action" value="done" class="btn btn-sm btn-success" onclick="return confirm('Mark this appointment as Done?');">
