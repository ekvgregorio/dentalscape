...
function getAdminOTPHistory($conn, $offset = 0, $perPage = 10) {
    $query = "SELECT aoh.*, a.role
              FROM admin_otp_history aoh
              JOIN admin a ON aoh.admin_id = a.id
        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div>
                        <h1 class="card-title">Admins OTP (One-Time Password) Usage</h1>
                        <h5 class="text-primary mt-2"><h5 class="text-primary mt-2"> </h5>
                    </div>
                </div>                         
...
<thead>
  <tr>
    ...
    <th>Timestamp</th>
                    ...
                    <tbody>
<?php 
    $otpHistory = getAdminOTPHistory($conn, $offset, $perPage);
    if ($otpHistory) {
        while ($record = $otpHistory->fetch_assoc()): ?>
            <tr>
                <td><?php echo htmlspecialchars($record['otp_history_id']); ?></td>
                <td><?php echo htmlspecialchars($record['admin_id']); ?></td>
                <td>
                    <?php if ($record['role'] === 'admin'): ?>
                        <span class="status-badge bg-danger text-white">Admin</span>
                    <?php elseif ($record['role'] === 'doctor'): ?>
                        <span class="status-badge bg-success text-white">Doctor</span>
                    <?php elseif ($record['role'] === 'secretary'): ?>
                        <span class="status-badge bg-primary text-white">Secretary</span>
                    <?php else: ?>
                        <span class="status-badge bg-secondary text-white"><?= htmlspecialchars(ucfirst($record['role'])) ?></span>
                    <?php endif; ?>
                </td>

                <td><?php echo htmlspecialchars($record['otp']); ?></td>
                <td><?php echo date('M d, Y H:i', strtotime($record['otp_expiry'])); ?></td>
                <td>
                    <span class="status-badge <?php echo ($record['is_used'] == 1) ? 'bg-success' : 'bg-warning'; ?> text-white">
                        <?php echo ($record['is_used'] == 1) ? 'Used' : 'Unused'; ?>
                    </span>
                </td>
        ...
        echo '<tr><td colspan="7">No OTP records found.</td></tr>';
    }
?>
