...
<tbody>
    <?php 
        $lockoutHistory = getUserLockedOut($conn); 
        if ($lockoutHistory) {
            foreach ($lockoutHistory as $record): ?>
                <tr>
                    <td><?php echo htmlspecialchars($record['lockout_id']); ?></td>
                    ...
                    <td>
<span class="status-badge bg-warning text-dark" style="font-size: 10px;">
    Locked Out
</span>
                    ...
                    <td><?php echo htmlspecialchars($record['lockout_type']); ?></td>
                    <td><?php echo htmlspecialchars($record['neighbourhood']); ?>, <?php echo htmlspecialchars($record['municipality']); ?></td>
                                        <td><?php echo date('M d, Y', strtotime($record['lockout_time'])); ?></td>
                                                            <td><?php echo date('H:i', strtotime($record['lockout_time'])); ?></td>
                </tr>
            <?php endforeach;
        } else {
            echo '<tr><td colspan="9">No lockout records found.</td></tr>';
        }
    ?>
</tbody>
